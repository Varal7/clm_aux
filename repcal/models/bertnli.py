# Reproduced from https://github.com/ysmiura/ifcc
...
class BERTNLI(torch.nn.Module):
    ...
    def __init__(self, name_or_path, bert_type="bert", cls="linear", length=128, force_lowercase=False, device="cpu", verbose=False):
        super().__init__()
    ...
    def to(self, device, *args, **kwargs):
        print('Moving BERTNLI to {0}'.format(device))
        self = super().to(device, *args, **kwargs)
        self.device = device
        self.bert = self.bert.to(device, *args, **kwargs)
        return self
    ...
    def train_step(cls, logits, gold_labels, optimizer, model=None, grad_clip=None):
        ...
        for i, gold_label in enumerate(gold_labels):
            if gold_label == "entailment":
                ...
                raise ValueError(f"Unknown label: {gold_label}")
    ...
    def forward(self, sent1s, sent2s):
        ...
        for idx, tokens in enumerate(buffer):
            ...
            seg_ids.append(torch.tensor([0 if i < boundaries[idx] else 1 for i in range(max_len)]))

        token_ids = torch.stack(token_ids).to(self.device)
        attn_mask = torch.stack(attn_mask).to(self.device)
        seg_ids = torch.stack(seg_ids).to(self.device)

        reps, cls = self.bert(token_ids, attention_mask=attn_mask, token_type_ids=seg_ids).values()
        if self.cls == "linear":
